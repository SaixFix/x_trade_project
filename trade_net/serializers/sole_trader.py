...
class SoleTraderCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            sole_trader = SoleTrader.objects.create(**validated_data)
            address, _ = Address.objects.get_or_create(**self._address)
            product, _ = Product.objects.get_or_create(**self._product)
            sole_trader.address = address
            sole_trader.product.add(product)
            sole_trader.save()

        return sole_trader


class SoleTraderListSerializer(serializers.ModelSerializer):
...
class SoleTraderUpdateSerializer(serializers.ModelSerializer):
    ...
    def save(self):
        ...
        sole_trader = super().save()

        with transaction.atomic():
            if self._address:
                if sole_trader.address is None:
                    address, _ = Address.objects.get_or_create(**self._address)
                else:
